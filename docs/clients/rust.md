...
The `check_health()` method performs a unary health check against the server using the standard gRPC Health Checking protocol.

::: tabs
== sync
```rust
fn check_health(&self, service: &str) -> DcbResult<ServingStatus>
```
== async
```rust
async fn check_health(&self, service: &str) -> DcbResult<ServingStatus>
```
:::

The `check_health()` method queries the health status of the server or a specific service.

### Parameters

| Name      | Type   | Description                                                                                                                         |
|-----------|--------|-------------------------------------------------------------------------------------------------------------------------------------|
| `service` | `&str` | Service name to check. Use an empty string (`""`) for overall server health, or a fully-qualified gRPC service name (e.g. `"umadb.v1.DCB"`). |
...
The `watch_health()` method opens a server-streaming health watch for the given service name.

::: tabs
== sync
```rust
fn watch_health(&self, service: &str) -> DcbResult<SyncHealthWatch>
```
== async
```rust
async fn watch_health(&self, service: &str) -> DcbResult<AsyncHealthWatch>
```
:::

The `watch_health()` method opens a stream of health status updates for the server or a specific service.
The server immediately sends the current serving status and then pushes updates whenever the status changes.
The returned [`SyncHealthWatch`](#health-watch) or [`AsyncHealthWatch`](#health-watch) is registered with the client's stream registry and will be automatically cancelled when the client is closed or dropped.

### Parameters

| Name      | Type   | Description                                                                                                                         |
|-----------|--------|-------------------------------------------------------------------------------------------------------------------------------------|
| `service` | `&str` | Service name to watch. Use an empty string (`""`) for overall server health, or a fully-qualified gRPC service name (e.g. `"umadb.v1.DCB"`). |

### Return Value

Returns a [`SyncHealthWatch`](#health-watch) or [`AsyncHealthWatch`](#health-watch) instance that yields [`ServingStatus`](#serving-status) updates.
...
| `NotServing`     | The server is not ready to handle requests.                                            |
| `ServiceUnknown` | The requested service is not known, used by [`watch_health()`](#watching-health) only. |


## Health Watch

The `SyncHealthWatch` and `AsyncHealthWatch` structs provide a stream of [`ServingStatus`](#serving-status) updates.

Both structs provide the following methods:

| Method                            | Description                                                                                                     |
|-----------------------------------|-----------------------------------------------------------------------------------------------------------------|
| `next()`                          | Returns `DcbResult<Option<ServingStatus>>`, waiting for the next status update, or `None` if the stream ended. |
| `next_timeout(timeout: Duration)` | Waits up to `timeout` for the next status update. Returns `Err(DcbError::Timeout())` if timed out.             |
| `cancel()`                        | Explicitly cancels this individual health watch stream.                                                         |
...
| `CancelledByUser()`            | Operation or stream was cancelled by the user.      |
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Check server health
    let health_status = client.check_health("")?;
    println!("Server health status: {:?}", health_status);

    // Watch server health
    let mut health_watch = client.watch_health("")?;
    if let Some(status) = health_watch.next()? {
        println!("Watched health status: {:?}", status);
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Watch server health
    let mut health_watch = client.watch_health("").await?;
    if let Some(status) = health_watch.next().await? {
        println!("Watched health status: {:?}", status);
    }
